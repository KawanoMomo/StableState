...
### 遷移線の自動再配置（Auto-Route）

ツールバーの「Auto-Route」ボタンで全遷移線を最適に再ルーティングする。手動配置後の仕上げとして使う想定で、処理時間がかかっても品質を優先する。

**最適化の目標（優先順）:**
1. 状態ボックスとの重なりを回避
2. 遷移線同士の交差数を最小化
3. 同一辺から出る遷移のポート位置を均等分散
4. 経路の総折れ回数を最小化（シンプルな経路を優先）
5. ラベル（イベント名/ガード/アクション）が他の要素と重ならない位置に配置

**アルゴリズム概要:**
1. 全遷移線のポート割り当てをリセット
2. 各遷移について、使用可能な辺の組み合わせ（上/下/左/右 × 上/下/左/右）を列挙
3. 各組み合わせで経路候補を生成し、状態ボックスとの重なりをチェック
4. 交差数が最小になるポート割り当ての組み合わせを探索（遷移数が少ない場合は全探索、多い場合はグリーディ+局所改善）
5. 確定した経路でラベル配置を最適化（折れ線の中間セグメント上で、他要素と重ならない位置）

**UI:**
- ツールバーに「Auto-Route」ボタン（Phase 1のルーティングが安定した後に追加、Phase 2）
- 実行中はプログレス表示（遷移数が多い場合、数秒かかる可能性がある）
- 実行後はUndo可能（実行前のDSLスナップショットを保持）
...
- 遷移線の自動再配置（Auto-Route）: 交差最小化・重なり回避の最適ルーティング
