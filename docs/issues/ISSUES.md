...
| ISSUE-004 | 改善 | MEDIUM | Open | ドラッグ/リサイズ操作にRAF最適化が未適用 | [ISSUE-004.md](ISSUE-004.md) |
| ISSUE-005 | 機能追加 | MEDIUM | Open | 矢印キー移動・接続管理パネルが未実装 | [ISSUE-005.md](ISSUE-005.md) |
...
| ISSUE-008 | 機能追加 | HIGH | Open | 検索/フィルタ機能が未実装 | - |
| ISSUE-009 | 機能追加 | HIGH | Open | 遷移線幅カスタマイズが未実装 | - |
| ISSUE-010 | 機能追加 | MEDIUM | Open | 遷移線スタイル（実線/破線）が未実装 | - |
| ISSUE-011 | 機能追加 | MEDIUM | Open | 遷移線カラー設定が未実装 | - |
| ISSUE-012 | 機能追加 | MEDIUM | Open | PlantUMLエクスポートが未実装 | - |
| ISSUE-013 | 改善 | LOW | Open | ID自動補正機能（labelToId・ID補正ボタン）が未実装 | - |
| ISSUE-014 | 改善 | LOW | Open | ポート間隔が固定値で接続数に応じた動的調整が未実装 | - |
| ISSUE-015 | 改善 | LOW | Open | 注釈ボタンが空データでも初期アクティブ表示される | - |
| ISSUE-016 | 機能追加 | LOW | Open | ハイライトモード（Hキー、孤立ステート検出）が未実装 | - |
| ISSUE-017 | 機能追加 | LOW | Open | @includeディレクティブが未実装 | - |
| ISSUE-018 | 機能追加 | LOW | Open | Git Visual Diff（VSCode拡張）が未実装 | - |
| ISSUE-019 | 機能追加 | LOW | Open | MCPサーバーが未実装 | - |
